...
def rk3_alt_step_func(func, t0, dt, t1, y0, f0=None, perturb=False):

    k1 = f0
    if k1 is None:
        k1 = func(t0, y0, perturb=Perturb.NEXT if perturb else Perturb.NONE)
    k2 = func(t0 + dt * _one_two, y0 + dt * k1 * _one_two)
    k3 = func(t0 + dt * _one_two, y0 + dt * k2 * _one_two)
    k4 = func(t1 , y0 + dt * k3/2)

    return (k1 + 4*k2+k3) * dt /6
...
def rk4_alt_step_func(func, t0, dt, t1, y0, f0=None, perturb=False):
    ...
    aaa= (k1 + 3 * (k2 + k3) + k4) * dt * 0.125
    return (k1 + 3 * (k2 + k3) + k4) * dt * 0.125

def rk5_alt_step_func(func, t0, dt, t1, y0, f0=None, perturb=False):

    k1 = f0
    if k1 is None:
        k1 = func(t0, y0, perturb=Perturb.NEXT if perturb else Perturb.NONE)
    k2 = func(t0 + dt * _one_four, y0 + dt * k1 * _one_four)
    k3 = func(t0 + dt * _one_four, y0 + dt * k2 * _one_four)
    k4 = func(t0+ dt * _one_two, y0 + dt * (k3/2))
    k5 = func(t0+dt*(3/4), y0 + dt*k4*(3/4))
    k6 = func(t0+dt , y0+k5)
    return (k1 + 2*k2+2*k3+k4+k5+k6) * dt /6
