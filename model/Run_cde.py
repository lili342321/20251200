...
from model.BasicTrainer_cde import Trainer,nvidia_info
...
init_gpu_per = used/tot
init_gpu = used
print("Initial usage of GPU0:", init_gpu)
print("Initial GPU0 utilization rate:", init_gpu_per)
...
#data
args.add_argument('--train_ratio', default=config['data']['train_ratio'], type=float)
...
args.add_argument('--init_len',default=config['train']['init_len'], type=int)
args.add_argument('--Rungokuta_order',default=config['train']['Rungokuta_order'], type=int)
args.add_argument('--feature_num',default=config['train']['feature_num'], type=int)
args.add_argument('--linear1', default=config['train']['linear1'], type=int)
args.add_argument('--init_len_tmp', default=config['train']['init_len_tmp'], type=int)
...
save_name = time.strftime("%m-%d-%Hh%Mm")+args.comment+"_"+ args.dataset+"_"+ args.model+"_"+ args.model_type+"_"+"lr{"+str(args.lr_init)+"}"+"wd{"+str(args.weight_decay)+"}"
path = '../runs'
...
train_loader, val_loader, test_loader,  times,scaler = get_dataloader_cde(args,
...
if args.loss_func == 'mask_mae':
    ...
    loss = torch.nn.HuberLoss(delta=0.5).to(args.device)
else:
    raise ValueError
...
trainer = Trainer(model,  loss, optimizer, train_loader, val_loader, test_loader,
                  ...
                  w,args.init_len,scaler,init_gpu,init_gpu_per)
if args.mode == 'train':
    trainer.train()
elif args.mode == 'test':
    model.load_state_dict(torch.load('./pre-trained/{}.pth'.format(args.dataset)))
    print("Load saved model")
    trainer.test(model, trainer.args, test_loader,  trainer.logger, times)
else:
    raise ValueError
